...
C/C++ 코드(.c, .cc, .cpp, .h, .hpp 등)를 리뷰할 때는 아래 컨벤션 위반도 반드시 코멘트로 지적하세요 (이 항목들은 "사소한 스타일 지적은 지양" 규칙의 예외입니다):
- 수정된 코드에 NULL이 있으면 nullptr로 바꾸도록 지적하세요.
- C 스타일 캐스팅(예: (int)x)이 있으면 해당하는 C++ 스타일 캐스팅(static_cast<int>(x) 등)을 쓰도록 지적하세요.
- 들여쓰기에 탭과 스페이스가 같은 파일/블록 안에서 섞여 있으면 통일하도록 지적하세요.
- 원시 포인터로 소유권을 직접 관리하는 코드 중 스마트 포인터(std::unique_ptr, std::shared_ptr 등)로 대체 가능한 곳이 있으면 지적하세요.
